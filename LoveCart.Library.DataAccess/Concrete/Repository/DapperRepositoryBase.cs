...
using System.Text;
using Dapper;
using LoveCart.Library.DataAccess.Util;
using LoveCart.Library.Utilities.Extensions;
using Ragnarok.Library.Utilities.Extensions;

namespace LoveCart.Library.DataAccess.Concrete.Repository;
...
public abstract class DapperRepositoryBase : IGenericRepository
{
    ...
    public async Task<IEnumerable<T>> GetAllAsync<T>()
    {
        ...
        return await connection.QueryAsync<T>($"SELECT * FROM {typeof(T).GetTableName()}");
    }
    ...
    public async Task<BaseResponse<List<T>>> SelectAsync<T>(Expression<Func<T, bool>> where)
    {
        ...
        {
            return await Task.FromResult(new BaseResponse<List<T>>(default, false) { error = new Error { exception = e }, Success = false });
        }
    ...
    public async Task<BaseResponse<T>> FirstAsync<T>(Expression<Func<T, bool>> where)
    {
        try
        {
            ...
            return await Task.FromResult(new BaseResponse<T>(rslt, true));
        }
        catch (Exception e)
        {
